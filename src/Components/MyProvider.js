  ...
  const addToCart = (Id) => {
    ...
    if (Cart) {
      if (Cart.find((u) => u.userId === User.userId)) {
        ...
        if (Cart.find((up) => up.userId === User.userId && up.productId === Id)) {
//***************both user and product*********************************
          let product = Cart.find((up) =>  up.userId === User.userId && up.productId === Id);
          let quantity = product.quantity + 1;
          console.log("Cart",Cart);
          var index =Cart.findIndex((up) =>  up.userId === User.userId && up.productId === Id);
          console.log(index);
          var pi=Cart[index];
          console.log(pi);
        ...
        console.log(Cart);
        let cart=[...Cart,{userId, productId, quantity, name, price, image}]
...
//*************************user but no product*********************************
  ...
  console.log("user but no product",cart);
          setCart(cart);
        }
      } else {
//**************************no user add product***********************************
...
console.log("no user no product",cart);
        setCart(cart);
